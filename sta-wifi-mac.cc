...
StaWifiMac::TryToEnsureAssociated (void)
{
  ...
  switch (m_state)
    ...
    case BEACON_MISSED:
       ...
       */
       printf("STA:BEACON_MISSED\n");
      m_linkDown ();
      SetState (WAIT_PROBE_RESP);
      SendProbeRequest ();
      break;
    case WAIT_ASSOC_RESP:
        printf("STA:WAIT_ASSOC_RESP\n");
...
StaWifiMac::Receive (Ptr<Packet> packet, const WifiMacHeader *hdr)
{
  ...
  else if (hdr->IsProbeResp ())     // if the received packet is for proberesponse
    {
      if (m_state == WAIT_PROBE_RESP)
        ...
        /*
          ...
          if (!probeResp.GetSsid ().IsEqual (GetSsid ()))
            {
              ...
              //proberesp.GetSsid() = received
              return;
            }
          SetBssid (hdr->GetAddr3 ());  // bssid -> sender ap mac address
          ...
          SetState (WAIT_ASSOC_RESP);   // move WAIT_PROBE_RESP to WAIT_ASSOC_RESP
          SendAssociationRequest (); 
          */
          ...
          if(count_proberesp >= 3 || m_probeRequestEvent.IsExpired()){
            Mac48Address tmp1 = list[0];
            int tmp1_count = 0, other;
          
            for(int i = 0; i<count_proberesp; i++){
                if(tmp1 == list[0])
                    tmp1_count++;
                else
                    other = i;
          ...
          list[count_proberesp] = hdr->GetAddr3();
          count_proberesp++;
